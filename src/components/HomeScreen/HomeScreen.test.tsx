...
jest.mock('@/helpers', () => ({
  ...
  loadUfoCaught: jest.fn(),
  saveUfoCaught: jest.fn(),
}));

const mockedLoadUfoCaught = loadUfoCaught as jest.Mock;
const mockedSaveUfoCaught = saveUfoCaught as jest.Mock;

// `waitFor(() => expect(mock).toHaveBeenCalled())` only proves `loadUfoCaught()` was invoked,
// not that its `.then(setUfoCaught)` has actually applied — flush the microtask queue for real.
const flushMicrotasks = () => act(async () => { await Promise.resolve(); await Promise.resolve(); });

beforeEach(() => {
  jest.clearAllMocks();
  mockedLoadUfoCaught.mockResolvedValue(true);
  mockedSaveUfoCaught.mockResolvedValue(undefined);
});
...
describe('HomeScreen — UFO settings button', () => {
  ...
  it('spins once when the random pause lands on UFO_SPIN_PAUSE_S', async () => {
    mockedLoadUfoCaught.mockResolvedValue(false);
    const timingSpy = jest
      .spyOn(Animated, 'timing')
      .mockReturnValue({ start: (cb?: (result: { finished: boolean }) => void) => cb?.({ finished: true }), stop: jest.fn() } as unknown as Animated.CompositeAnimation);
    // Math.random() close to 1 always picks the last UFO_PAUSE_OPTIONS_S entry (UFO_SPIN_PAUSE_S).
  ...
  it('does not re-center the rotation when the spin animation itself is interrupted', async () => {
    ...
    // ufoRotation, which gets interrupted (finished: false) — its callback must be a no-op.
    let call = 0;

 ...
 * reasoning as a country's own Contour data living on its `countries.json` row (see
 * `ContourDataRow`): a judgment call specific to this game (e.g. an island city that reads badly
 * pinned onto the mainland-only outline), not a property of the place itself. Keyed by
 * `${code}:${name}` (see `excludeKey`) since `Place` has no stable id.
 */
